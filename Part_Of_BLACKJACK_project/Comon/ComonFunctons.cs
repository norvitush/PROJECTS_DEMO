...
public static class Helpers
{
    ...
    public static void Clear(this Transform transform, params Type[] excludeComponentsOfType )
    {
        ...
        for (int i = 0; i < cnt; i++)
        {
            ...
            if(canDestroyed) GameObject.Destroy(transform.GetChild(i).gameObject);
        }
    }

}

namespace GoldenSoft.Column21
{
    /// <summary>
    ...
    public static class CFGameObjects
    {
        ...
        public static bool HasComponent(this GameObject self, Type testComponent)
        {
            return self.GetComponent(testComponent);
        }
        public static bool HasComponent<T>(this GameObject self, out T component)
        {
            if (self == null) { component = default;  return false; }
...
public static class CFMove
    ...
    static class CFVectors
    {
        ...
        public static Vector2 WorldCamMin(this Camera thisCamera, float distance = 0f)
        {
            if (distance == 0f)    distance = thisCamera.nearClipPlane;
            
            return thisCamera.ViewportToWorldPoint(new Vector3(0, 0, distance));
        }
        public static Vector2 WorldCamMax(this Camera thisCamera, float distance = 0f)
        {
            if (distance == 0f) distance = thisCamera.nearClipPlane;

            return thisCamera.ViewportToWorldPoint(new Vector3(1, 1, distance));
        }
        
        public static float WorldCamWidth(this Camera thisCamera, float distance = 0f)
        {
            return thisCamera.WorldCamMax(distance).x - thisCamera.WorldCamMin(distance).x;
        }
        public static float WorldCamHeigth(this Camera thisCamera, float distance = 0f)
        {
            return thisCamera.WorldCamMax(distance).y - thisCamera.WorldCamMin(distance).y;
        }
        public static Vector2 WorldCamSize(this Camera thisCamera, float distance = 0f)
        {
            Vector2 min = thisCamera.WorldCamMin(distance);
            Vector2 max = thisCamera.WorldCamMax(distance);
